...
float paramF1 = 53;
float paramF2 = 60;
float paramBaudRate = 2;
float paramFreqADC = 10000.0;
int paramKu = 1600;
float paramPorog = 10;
...
//open lib for E154
  handle = dlopen("liblcomp.so",RTLD_LAZY);
   ...
   complete=0;
   //pI->EnableCorrection();	не реализована для платы Е154
   pI->InitStartLDevice();
...
//    print adc param
   ...
   wmove(wdebug,2,55);
   wprintw(wdebug,"paramBaudRate:	%5.2f\n",paramBaudRate);
   wmove(wdebug,3,55);
...
//filter init
    ...
    iirfilt_cccf q3 = iirfilt_cccf_create_prototype(ftype, LIQUID_IIRDES_LOWPASS, format, order, 2*paramBaudRate/paramFreqADC, 0, Ap, As);
...
long long ind = 0;
int	 ovfd_counter =	0;	//overflow detector counter
int	 ovfd =	0;
...
while(thread_work)
{
    if(complete==1)
   {
     ...
     for (i=0; i<=4096; i++) 
     {		
//      input signal with amplitude correction
     	x = std::complex<float>(*(bufferADC+i)*paramKu/2048, 0);
// 	overflow detect
	if (abs(x)==paramKu)
	{
	  ovfd_counter++;
