...
//    print adc param
   ...
   wmove(wdebug,21,10);
   wprintw(wdebug,"paramBoudRate:		%5.0f\n",paramBoudRate);
   wmove(wdebug,22,10);
...
void init_windows(void){
  ...
  wdebug=subwin(stdscr,10,80,19,0);
...
//filter init
    ...
    float fc1=f01+paramBoudRate*0.5/paramFreqADC;          // cutoff frequency
    float fc2=f02+paramBoudRate*0.5/paramFreqADC;          // cutoff frequency
    float Ap=1.0f;          // pass-band ripple
    float As=60.0f;         // stop-band attenuation
    float samplenum=paramFreqADC/paramBoudRate;
    ...
    iirfilt_cccf q3 = iirfilt_cccf_create_prototype(ftype, LIQUID_IIRDES_LOWPASS, format, order, paramBoudRate/paramFreqADC, 0, Ap, As);
